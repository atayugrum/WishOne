    ...
    try {
        ...
        if (genAI && (data.title || data.description)) {
            try {
                ...
                const prompt = `Analyze: ${data.title} ${data.description.substring(0, 200)} ${data.price}. Cats: ${JSON.stringify(CATEGORY_MAP)}. Output JSON: { "category": "String", "subcategory": "String", "cleanTitle": "String", "priorityLevel": "String", "priorityLabel": "String", "reason": "String", "estimatedPrice": Number }`;
...
app.post('/api/ai/combo-suggestions', async (req, res) => {
    ...
    try {
        ...
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
        const prompt = `Stylist Task: Create 1-3 outfits from: ${JSON.stringify(closetItems.map(i => ({ id: i.id, name: i.title, cat: i.category })))}. Output JSON: { "combos": [{ "name": "String", "description": "String", "itemIds": ["id1"] }] }`;
        const result = await model.generateContent(prompt);
...
app.post('/api/ai/purchase-planner', async (req, res) => {
    ...
    try {
        ...
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
        const prompt = `Budget: ${budget} ${currency}. Items: ${JSON.stringify(wishlistItems.map(i => ({ id: i.id, name: i.title, price: i.price, priority: i.priority })))}. Pick items. Output JSON: { "recommendedItems": [{ "itemId": "String", "reason": "String" }], "summary": "String" }`;
        const result = await model.generateContent(prompt);
        res.json(JSON.parse(result.response.text().replace(/```json|```/g, '').trim()));
    } catch (e) {
...
app.post('/api/ai/reaction', async (req, res) => {
    try {
        const reactions = ["That's a vibe.", "Great choice.", "Adding this to the manifest.", "Love that color.", "This will spark joy."];
        res.json({ message: reactions[Math.floor(Math.random() * reactions.length)] });
    } catch (error) { res.status(500).json({ error: 'AI reaction failed' }); }
});
...
app.post('/api/ai/moodboard', async (req, res) => {
    ...
    try {
        ...
        if (!genAI) return res.status(503).json({ error: 'AI unavailable' });
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
        const prompt = `Context: Moodboard titled "${title}". Existing: ${existingPins?.length || 0}. Task: Suggest 3-4 product TYPES. Output JSON: { "suggestions": [{ "name": "String", "why": "String" }] }`;
        const result = await model.generateContent(prompt);
        res.json(JSON.parse(result.response.text().replace(/```json|```/g, '').trim()));
...
app.post('/api/ai/compatibility', async (req, res) => {
    ...
    try {
        ...
        if (!genAI) return res.status(503).json({ error: 'AI unavailable' });
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });
        const prompt = `Analyze compatibility. A (${names.user}) items count: ${userItems.length}. B (${names.friend}) items count: ${friendItems.length}. Output JSON: { "summary": "String", "score": Number, "sharedInterests": ["Tag1"] }`;
        const result = await model.generateContent(prompt);
        res.json(JSON.parse(result.response.text().replace(/```json|```/g, '').trim()));
